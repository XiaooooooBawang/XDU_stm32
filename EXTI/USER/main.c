...
{
    /*
     * )EXTI 中断配置函数,ExIntInit 函数配置 EXTI 信号源为 PC13，下降沿触发。
     */
    RCC->AHB1ENR |= 1<<2; //使能 GPIOC 时钟，外设时钟使能寄存器AHB1ENR第2位赋1使能
    RCC->APB2ENR |= 1<<14; //使能 SYSCFG 时钟，系统配置控制器时钟使能寄存器APB2ENR第14位赋1使能
    SYSCFG->EXTICR[3] = (SYSCFG->EXTICR[3] & ~(0xF<<4))| (2<<4); //外部中断配置寄存器3的第7-4位赋0010，配置 EXTI13 信号源为 PC13
    EXTI->IMR |= 1<<13; //中断屏蔽寄存器第13位赋1，取消对 EXTI13 信号线的屏蔽
    EXTI->FTSR |= 1<<13; //下降沿触发器选择寄存器第13位赋1，设定 EXTI13 中断触发信号为下降沿
    NVIC->ISER[1] |= 1<<8; //在中断设置使能寄存器 NVIC 中设置 EXTI15_10 中断使能
}
...
void EXTI15_10_IRQHandler(void)
{
     ...
     * 中断处理函数
     */
    EXTI->PR |= 1<<13; //在挂起寄存器第13位赋1，清除当前已经产生的 EXTI13 中断
